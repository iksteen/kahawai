...
tokio::task_local! {
    /// The server-generated id for the request currently being polled.
    ///
    /// It is task-local rather than copied through every handler and error
    /// constructor. Axum turns a handler's return value into a response while
    /// polling the same future, so an `ApiError` and an error embedded in an
    /// otherwise successful body both see the id installed by
    /// [`request_context`].
...
fn request_id() -> String {
    REQUEST_ID
        ...
        // Direct unit tests and library callers can construct a body without
        // a router. It still gets a valid id; a served response is overwritten
        // with the request's id in `IntoResponse` below.
        .unwrap_or_else(|_| ulid::Ulid::generate().to_string())
}

/// Install the correlation boundary around one HTTP request.
///
/// The id is always generated here. An inbound `X-Request-Id` is deliberately
/// ignored: reflecting an attacker-controlled value into privileged logs
/// would make correlation ambiguous and turns a log field into untrusted
/// content. The response header is useful for successful requests too, while
/// every refusal additionally carries the same value in its JSON body.
...
pub enum ErrorCode {
    ...
    /// The on-disk development web bundle is between Vite's remove and
    /// replace steps. The same asset request is expected to work shortly.
    WebUnavailable,
...
pub struct ApiErrorBody {
    ...
    message: String,
    /// Correlates the generic client answer with the complete server-side
    /// cause. Generated by the hub; never copied from a request header.
    request_id: String,
}
...
impl IntoResponse for ApiError {
    fn into_response(mut self) -> Response {
        // Normally captured by `new`; doing it again here makes the invariant
        // hold even if an error was constructed before entering the request
        // future and handed into it over a channel.
...
mod tests {
    ...
    fn the_body_is_json_with_a_snake_case_code() {
        let body = serde_json::to_value(ApiErrorBody::new(
            ErrorCode::SourceOffline,
            "the host holding this file is not connected",
        ))
        .unwrap();
        assert_eq!(body["code"], "source_offline");
        assert_eq!(
            body["message"],
            "the host holding this file is not connected"
        );
        let id = body["request_id"].as_str().expect("a request id");
