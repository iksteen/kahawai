...
pub async fn project_files(
    ...
    compose_library(registry).await;
}

pub async fn compose_library(registry: &kahawai_hub::registry::Registry) -> String {
    if let Some(library) = registry.catalogue().libraries().await.unwrap().first() {
        return library.id.clone();
    }
    let collections = registry.catalogue().collection_summaries().await.unwrap();
    let kind = collections[0].collection.media_type;
    let ids = collections
        .into_iter()
    ...
    registry
        .catalogue()
        .create_library("Fixture", kind, &ids)
        .await
        .unwrap()
}
