...
async fn anime_collection_resolves_movies_but_not_extras() {
    ...
    let movie: Option<(String, Option<i64>)> =
        sqlx::query_as("SELECT title, year FROM collection_items WHERE kind = 'movie'")
            .fetch_optional(&db)
            .await
            .unwrap();
    ...
    let nc: Option<(Option<i64>, i64, String)> = sqlx::query_as(
        "SELECT i.season,i.episode,p.title
           ...
           JOIN collection_items i ON i.id=fb.item_id JOIN collection_items p ON p.id=i.parent_id
