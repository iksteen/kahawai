...
async fn admin_flow_enrollments_satellites_archive_restore() {
    ...
    let api = kahawai_hub::api::router(registry.clone(), auth, sessions, enrollments.clone(), Arc::new(kahawai_hub::subtitles::Subtitles::new(tempfile::tempdir().unwrap().keep())), Arc::new(kahawai_hub::artwork::Artwork::new(tempfile::tempdir().unwrap().keep())));
