...
The global preference has three states: the empty/default value applies gain only when negotiation already encodes audio, `off` suppresses it, and `force` asks negotiation to replace measured direct/copied audio with an encode. Force is admitted only for a single-part source, an exact measured output layout, a compatible audio encoder, and an unchanged video mode. The hub preflights its local AAC/Opus layout before replacing direct/copy; an unsupported layout retains the ordinary plan rather than paying for a no-op transcode. Full transcoders receive the exact layout map from protocol minor 5. Minor-4 scalar fields remain wire-compatible for old hubs, but current placement never relies on them: an executor may fold a nominal 7.1 source to 5.1, so source channel count cannot prove which scalar will apply. Default normalization re-probes against an exact-gain-compatible worker but accepts the result only when the complete video path (codec, container, filters and subtitle burn) is unchanged. If no such worker can run the ordinary encode—or final ASS/capacity placement races the probe—placement retries the original plan without optional gain, so earlier peers remain usable without normalization. Force likewise falls back to the ordinary plan rather than paying for a no-op encode. Explicit `mode=direct` remains original bytes.

Protocol thresholds live in `kahawai_proto::ProtocolFeature`; planners and registries carry a typed required feature rather than comparing minor numbers. The exact-gain requirement is repeated on the final `PlacementNeed`, not only the earlier capability probe, because load/pace may choose a different box between planning and reservation. Track switches cannot move an existing session, so they likewise suppress unsupported normalization and undo a force-only encode. Scalar wire fields use protobuf presence, so an old hub's absent fields cannot become fabricated unity gain on a new transcoder. New hubs additionally emit the historical NaN/zero sentinels inside present fields when talking across minor skew, because a minor-4/5 generated decoder otherwise turns omission back into numeric zero.
...
**Job runners (MH-11/MH-13).** Urgent subtitle/image requests run immediately because a viewer is waiting. ED2K, declarations, geometry/keyframe probes and subtitle pre-warm remain idle-gated background work. Season analysis has its own one-at-a-time queue because its unit is a whole ordered season rather than one source; it waits while a scan or viewer lease is active and checks that gate between analyzer steps. Loudness has a separate one-file-at-a-time runner and shares the single background permit, but it is lower priority: queuing a watched-first season marks segment work pending, the loudness GStreamer callback releases the permit at its next audio-buffer checkpoint and blocks there, and the same pipeline reacquires the permit and resumes after the season. No audio is re-decoded, segment analysis does not overlap the bounded loudness meter fanout, and the paused pipeline's bounded queues are the memory cost of preserving completed work. File/track start, completion time and segment yield/resume are logged so a long decode is visible rather than appearing idle. The decoders themselves use short-lived native threads; on glibc their now-free per-thread arenas remained resident, observed as 798 MiB of private anonymous 64 MiB mappings while no segment job was alive. At 30-second decoder checkpoints and again at completed-season/completed-loudness-file boundaries, `malloc_trim(0)` returns whole free pages from every arena (glibc ≥2.8, MT-safe; [malloc_trim(3)](https://man7.org/linux/man-pages/man3/malloc_trim.3.html)). This is lifecycle reclamation, not a quota: live pipelines and their buffers are untouched, while non-glibc targets keep their native allocator behaviour. All runners keep control-link intake and heartbeats independent of blocking file work.
