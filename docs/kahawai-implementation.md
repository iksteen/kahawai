...
Priority is therefore fixed, with no waiting-time promotion: explicit playback/up-next/admin demand; catalogue freshness; attachment/chapter declaration; keyframes; geometry; local metadata/root resolution; ED2K; subtitle prewarm; segments; loudness. Viewer reads and viewer-triggered subtitle extraction register interactive exact-domain work immediately rather than waiting for capacity; their presence pauses conflicting scheduled work at its next checkpoint. FIFO and the existing useful order (for example movies then newest first for loudness) apply only within a scheduled type. Admission stays work-conserving while a higher job is blocked by some other resource; when that blocker clears, any newly conflicting lower job is interrupted at its next checkpoint. ED2K checks every hash chunk, sequential subtitle demux every bus cycle, scans within directory walks and stat batches, byte reads every 256 KiB, and loudness/season analysis at decoder callbacks. A byte-read permit is released before downstream delivery, so an idle lease or stalled client does not suppress background work. The permit follows spawned blocking work, so aborting its async waiter cannot accidentally admit a conflicting replacement while the old thread still reads. There is no second activity flag or special loudness-versus-segments rule.
