...
**Image subtitles — bitmap streaming before burn-in (HUB-32b).** PGS/VobSub get a third, cheaper tier that precedes burn-in in the policy order: the hub decodes the stream server-side (RLE display sets → RGBA tiles with palette applied) and serves it as a timed bitmap track — PNG tiles plus a timing/position manifest fetched alongside the media, sourced through the same HUB-34 ladder. Clients declaring `graphics_overlay: true` (the web player does — it's the same canvas the ASS renderer uses) composite the display sets themselves: full visual fidelity, zero video decode/encode/blend on the server, so a PGS-subtitled direct-play or remux stays transcoder-free. Burn-in remains only for clients that can't composite an overlay. MP4's `mp4s` VobSub is a distinct wire shape from Matroska: `qtdemux` turns its binary ESDS palette into a `dvd-spu-clut-change` event instead of putting `.idx` text in `codec_data`; the live tap consumes that event and converts its limited-range BT.601 Y'CrCb entries to RGB. Those files carry no canvas size, and some muxers rewrite SPU coordinates into the HD video's coordinate space, so a rectangle crossing DVD's 720×576 bounds uses the video caps as its canvas rather than being enlarged from a false DVD canvas.
...
**Subtitle prewarm is a queue with two kinds; OCR is not a queue (2026-09-20).** Text prewarm used to re-derive "what is missing" every round by walking every file of every connected collection and stat'ing a cache file per track, then pushing a list the mediahost held only in memory; nothing woke it when a scan landed. It is now mediadb's `subtitle_jobs`: one row per probed file and kind, created and reset by the catalogue commit itself in `apply_catalogue` (a byte change resets a row; a metadata-only version bump does not), worked by the hub's shared queue driver (`hub/queue.rs`: claim and work while there is something to do, otherwise sleep until the earliest due time or a wake, with a slow tick as lost-event insurance; enrichment runs on the same driver and lost its 2 s poll). The two kinds share one mechanism. `text` is the embedded text tracks, walked out of the container in one pass and settled by the host's `FileSubtitles`; `sets` is the image tracks' display sets, walked one track at a time and settled once every image track's sets have landed (`ImageSubtitles`). Each hub keeps a window of at most 16 outstanding file leases per kind per connected mediahost, shared across its collections, refilling to 16 when 8 or fewer remain. The durable backlog stays on the hub; image files can expand to multiple track requests. Eight queued files cover refill latency while the sixteen-file cap bounds satellite intake and scheduling cost, in exchange for more small messages and claim queries. Each refill ranks the remaining work from current watch state. Full windows sleep until a result wake or lease expiry, ignoring already-due pending rows; offline and unsupported hosts do not drive a busy loop. One step claims only the available window capacity, settles rows that need nothing (no such track, or everything cached — one `try_exists` per claimed track, never a walk), and offers the rest as one worklist message per batch: `SubsWorklist` for text, `ImageSubsWorklist` (protocol 4.4) for sets — one message per batch rather than one `ExtractImageSubs` per track, because the host's inbound request channel holds 32 and an overrun tears down the link; a 4.3 host is not offered `sets` at all and its rows wait, visibly pending, for the upgrade. Rows are queue state, not artifact state: the cache on disk stays the truth of "done", and a rebuilt mediadb costs one incremental pass of cheap claims. Ranking happens in the claim SQL from live watch state (`workorder::in_flight`). A reported extraction error retries after an hour and parks the row as `blocked` after six reports; a mediahost reconnect releases everything offered to it. The landing gate resolves a source through the catalogue (`Store::source_file`), so a VobSub sidecar's `.idx` path lands on the media file that lists it instead of being dropped as "not a file row" — which had been silently starving sidecar OCR. **OCR** is a function of one thing — display sets on disk with no `ocr.json` beside them — so it has no rows: when sets land, `link_service` hands the file to one worker that OCRs it while no session is playing (it waits on the sessions' idle signal, not a poll); at startup the worker is seeded once from the catalogue for sets that landed while the hub was down; a track whose OCR fails leaves a `-ocr.failed` marker and is skipped until an administrator reruns, which forgets the markers and seeds again; a track whose language has no Tesseract model is skipped without a marker. Wakes: catalogue commit (also wakes enrichment, which previously found new jobs only by polling), subtitle landings, mediahost reconnect. Schema meaning: the `kahawai-mediadb::subtitles_work` module doc.
...
**Lightweight sessions run entirely inside the hub** — this is why `kahawai-hub` depends on `kahawai-media`, and they run on the same `kahawai_playback::executor` a transcoder uses for dispatched work: one `<data_dir>/sessions/<session>/r<N>` directory per run, sockets under a short `/tmp/kahawai-XXXX`, a death watch that stores the worker's evidence when it dies mid-session, and pace samples folded into `transcoder_pace` under module `local` so `predict_local` has observations and not only the benchmark. `hub/sessions/local.rs` is the hub's side of that: the lease byte source and the TC-6 sink retry. For pure remux, the hub feeds the mediahost byte stream through a local demux-only pipeline (`appsrc ! parsebin ! <selected streams, no decode> ! cmafmux → hlssink3`-style segmenting). The same supervised pipeline may copy video while decoding and encoding audio for codec conversion or downmix. Both are cheap relative to video decode/filter/encode, need no placement, work with zero transcoders attached (AR-10), and keep full transcoders free for video work. Seek = pipeline restart at the target keyframe, same as §6.
...
**Scanner and authoritative catalogue.** The mediahost owns `catalog.db` in its state directory (WAL, foreign keys enabled). It uses the same serialized-writer invariant as the hub, with three query-only readers plus one actor-owned writer, retaining its former four-connection budget. Scans, claims, acknowledgements and derived results therefore share one FIFO write order while snapshot/delta reads remain concurrent. The configured collection name is its stable ID; its epoch changes only when the namespace is removed/recreated or changes media type, while every file, tombstone and derived fact increments a collection-local version in the same transaction. `catalog_files` is the stat/discovery fast path and seen-generation journal; `catalog_records` stores one current value per replicated entity plus deletion tombstones. A root token is `root-sha256-` plus unpadded base64url of the complete `SHA-256(utf8("kahawai-root-path-v1") || 0x00 || normalized_path_utf8)`. The source key is `(collection, root token, path_rel)` locally and gains the enrolled module ID only when a hub projects it. Each successful root walk reconciles its complete inventory before any media discovery: missing sources publish tombstones immediately, while unavailable or partially unreadable roots retain their prior rows. Watches install before startup scans are admitted to the storage scheduler, so changes during an initial scan are queued for follow-up reconciliation. Ordinary scans reuse stored probes and content hashes when media size/mtime match. Exact content-write notifications bypass that check only for the affected media file, covering writes within the stored timestamp resolution. Selected sidecars are refreshed separately; their paths, sizes and sub-second modification times form `MediaInfo.sidecar_revision`, so an in-place edit publishes new catalogue facts without re-reading the media bytes. Learning a missing initial fingerprint updates only the local comparison baseline in bounded batches; unchanged associations do not create replicated catalogue updates. New/changed media, stale music discovery generations and explicit user-requested deep scans run GStreamer discovery and container inspection. Logs distinguish actual probes (`scanned`) from sidecar refreshes (`refreshed`); progress counts both as processed files. Nothing waits for a hub commit.
...
**Universal scheduler (MH-11/MH-13).** Missing local facts, not hub worklists, select ED2K, loudness and segment work. One process-wide admission authority also owns scans, recursive watch installation, exact-source probes, and the filesystem operations behind standalone or all-in-one byte reads. A job requests one atomic bundle: zero or one CPU-heavy slot, every storage domain it will touch, and a named exclusive resource where the algorithm itself is singleton (season comparison). The default is one CPU-heavy slot and one operation per automatically detected filesystem device; configured domains can group paths backed by one NAS or raise concurrency for storage known to sustain it. This cost model distinguishes rebuild cost from point-of-use latency: scans restore authoritative truth; ED2K unlocks exact AniDB matching; subtitle prewarm unlocks OCR; segments add skipping; loudness is optional polish, while viewer and explicit admin work has somebody waiting now.

Priority is therefore fixed, with no waiting-time promotion: explicit playback/up-next/admin demand; catalogue freshness; attachment/chapter declaration; keyframes; geometry; local metadata/root resolution; ED2K; subtitle prewarm; segments; loudness. Resource requirements describe the work, independently of its priority. Scans, watch installation and bounded metadata/geometry probes request storage only; full-file hashing, segment detection and loudness request CPU as well. A scan may briefly decode to discover metadata, but it does not reserve the sustained-analysis CPU slot for its directory walk or bounded probes.

Viewer byte leases hold an interactive CPU reservation until they close, for both remote mediahosts and all-in-one reads, including gaps between reads and network backpressure. Storage guards cover actual path resolution, open/stat, seek and read operations only, and are released before downstream delivery. This lets scans and other storage work proceed between viewer reads while sustained CPU work waits; the cost is some shared disk bandwidth, in return for fresh catalogue data without sacrificing playback CPU. Viewer reads and on-demand extraction enter immediately and interrupt conflicting scheduled work at its next checkpoint. There are no job-type or priority exemptions: a hint may reorder work, but cannot bypass an occupied resource.
...
**Loudness backfill order.** The runner drains collection worklists into one file queue instead of consuming a 128-file message inline. Movie collections outrank series/anime; within either category, the local source's current `mtime` sorts newest first. It re-drains intake before every choice, so a newly announced movie can jump the queued series backlog after the current admitted file. The current pipeline is interruptible and resumes in place when higher conflicting work finishes. Queue cost is one local metadata stat per source plus an in-memory sort—no media read and no protocol field.

**A pause names its owner.** Scheduler records name the semantic type, exact source or collection, storage domains and optional hub owner. Link teardown cancels that owner's extraction, read and hint state; source-owned catalogue work has no hub owner and survives link churn.

**A decoder must make progress.** The loudness bus wakes every ten seconds and fails a track after 60 seconds without a decoded audio buffer. A callback currently blocked in a scheduler checkpoint is exempt, so an intentional preemption has no deadline. A selected Opus stream that produces neither buffers, EOS nor an error becomes a revision-guarded terminal measurement failure instead of holding the entire movie queue forever.

`OpenRead.background` remains for small hub-owned work such as NFO reads. Those reads acquire the local-metadata class per requested block; viewer reads register immediate demand for the lease's exact storage domain. The all-in-one short circuit supplies the same admission callbacks to its direct reader, so transport choice cannot bypass scheduling.

**ED2K hasher (MH-9).** Anime files missing a local `file_hashes` record are selected below metadata work and above subtitle/analysis polish because the hash unlocks exact AniDB matching. Full-file ED2K (9.28 MiB chunked MD4), optional filename CRC32 verification, and terminal read failures are committed to the local catalogue, revision-invalidated when bytes change, then projected to every hub. A projected terminal failure clears any prior hash for that exact source before logging the diagnostic, including when replacement bytes retained the same size and mtime. Scan completion never waits on this expensive rebuild work.

**File server.** Serves `OpenRead` leases only. Each protocol-4 request carries the persisted collection and required exact `SourcePath`; the mediahost resolves that one configured root and canonicalizes the candidate only for confinement, never searches other roots. Missing sources and empty/unknown root tokens are protocol errors. Each foreground path/open/stat/seek/read operation enters immediate exact-domain demand; background operations use scheduled local-metadata permits. Neither mode holds admission while a lease is idle or while a byte chunk waits on the network. Opens remain read-only; canonical confinement rejects `..` and symlink escapes (NFR-4).
...
[mediahost.scheduler]
cpu_slots = 1                              # simultaneous CPU-heavy jobs
[[mediahost.scheduler.io_domains]]
name = "tank"                              # optional; otherwise group by device
roots = ["/tank/movies", "/tank/music", "/tank/anime"]
max_concurrent = 1                         # work against this backing store
[[mediahost.hubs]]
