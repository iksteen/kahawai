...
[hub.subtitles]
cache_max_mb = 0                      # OPS-6: cap on the re-derivable subtitle cache
                                      # (extracted cues, fonts). 0 = uncapped, the
                                      # default. Downloaded subtitles are never
...
**Disk bounds (OPS-6).** A quota may only reclaim what the hub can rebuild *for free*: the extracted cue/ASS caches and font bundles under `subtitles/`, which a demux pass regenerates from the media file with no network call. An hourly janitor evicts those least-recently-used down to 90 % of `hub.subtitles.cache_max_mb` (0 = no cap, the default — the cache is small next to the media it describes, and the reflex to cap it is worth questioning before it costs anyone a rebuild). Everything else is excluded on purpose: **downloaded subtitles** are database-referenced, shared between every user of the item (HUB-23) and cost a scarce provider entitlement, so evicting one trades a rate-limited resource for a cheap one; **artwork and the AniDB dumps/XML** are provider-derived too, and a re-fetch is exactly the traffic §4.3's pacing exists to avoid. Session scratch is already bounded by session lifecycle (wiped at startup, per-session teardown, idle reaping). Hub stream proxying uses fixed bounded buffers (64 KiB chunks, bounded channel per session) so a slow client applies backpressure to the mediahost read instead of ballooning hub memory — this also caps per-session memory for the in-hub remuxer via `appsrc` `max-bytes`. Hub stream proxying uses fixed bounded buffers (64 KiB chunks, bounded channel per session) so a slow client applies backpressure to the mediahost read instead of ballooning hub memory — this also caps per-session memory for the in-hub remuxer via `appsrc` `max-bytes`.
