...
*So the burn arm uses the demuxer, and only the demuxer.* Fonts exist in exactly two places: as attachments inside the mkv, or nowhere. Taking the demuxer's own subtitle pad brings the attachments with it, needs no synthesis, and avoids feeding back our own reconstruction of the script — where feeding an extracted copy through `appsrc` would throw away attachments the demuxer was about to hand us. The extracted `.ass` the hub caches exists for external native players (HUB-34), not to be re-read by our own pipeline.

A genuine external `.ass` sitting beside the media therefore has no burn path, and does not need one: it is already text, so it client-renders or flattens, both of which sit ABOVE burn in the preference order. A user who explicitly picks burn for one gets the honest error rather than a silent flatten.
