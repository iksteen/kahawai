...
**Universal scheduler (MH-11/MH-13).** Missing local facts, not hub worklists, select ED2K, loudness and segment work. One process-wide admission authority also owns scans, recursive watch installation, exact-source probes, standalone byte leases and all-in-one direct reads. A job requests one atomic bundle: zero or one CPU-heavy slot, every storage domain it will touch, and a named exclusive resource where the algorithm itself is singleton (season comparison). The default is one CPU-heavy slot and one operation per automatically detected filesystem device; configured domains can group paths backed by one NAS or raise concurrency for storage known to sustain it. This cost model distinguishes rebuild cost from point-of-use latency: scans restore authoritative truth; ED2K unlocks exact AniDB matching; subtitle prewarm unlocks OCR; segments add skipping; loudness is optional polish, while viewer and explicit admin work has somebody waiting now.

Priority is therefore fixed, with no waiting-time promotion: explicit playback/up-next/admin demand; catalogue freshness; attachment/chapter declaration; keyframes; geometry; local metadata/root resolution; ED2K; subtitle prewarm; segments; loudness. FIFO and the existing useful order (for example movies then newest first for loudness) apply only within a type. Admission stays work-conserving while a higher job is blocked by some other resource; when that blocker clears, any newly conflicting lower job is interrupted at its next checkpoint. ED2K checks every 4 MiB, sequential subtitle demux every bus cycle, scans between bounded blocking batches, byte reads every 256 KiB, and loudness/season analysis at decoder callbacks. The permit follows spawned blocking work, so aborting its async waiter cannot accidentally admit a conflicting replacement while the old thread still reads. There is no second activity flag or special loudness-versus-segments rule.
