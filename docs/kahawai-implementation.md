...
- `FileRecord = { source: SourcePath { root_token, path_rel }, size, mtime, identity: ContentId, streams: StreamInfo[], sidecars[], tags{} }`; every source-bearing protocol-3 message uses this one required shape
...
**Measured audio loudness normalisation (HUB-38).** GStreamer's default `audioconvert` downmix protects against worst-case correlated channels by dividing the whole matrix by its largest coefficient sum; that headroom can make a 5.1→2.0 fold roughly 7–10 dB quieter. On the owning mediahost, one decode tees into `ebur128level` meters for the untouched native layout and for that exact stereo fold, once per physical file/audio stream and source revision. The hub stores both integrated-LUFS/true-peak pairs, derives static gains toward −18 LUFS (the centre of EBU R 128 s2's −20…−16 streaming range), and limits each so its measured linear signal stays at or below −1 dBTP per EBU R 128. After the encoder layout is pinned, the worker's ordinary `volume` element applies the stereo gain to stereo output or the native gain when a multichannel encode preserves the source channel count; other channel-count changes and direct/copy output remain untouched. Music collections are never queued or normalized here because their ReplayGain path is authoritative. A global `loudness_normalization=off` user preference suppresses every gain while retaining the shared measurements; absence is enabled, and the decision is snapshotted when a session starts so track-switch re-plans cannot change it underneath that session. No continuously adapting normalizer changes programme dynamics. Rebuild cost is one full audio decode per track, idle-gated and source-local; point-of-use cost is one indexed lookup and one constant multiplier. The session fact reports the applied dB value.

The global preference has three states: the empty/default value applies gain only when negotiation already encodes audio, `off` suppresses it, and `force` asks negotiation to replace a measured direct/copied audio stream with an encode. Force is audio-bounded: it is attempted only for a single-part source with a current measurement and accepted only when the video keeps the mode it had under ordinary negotiation. A missing/stale measurement, missing compatible audio encoder, multipart source, or container path that would introduce a video encode falls back to the ordinary plan. An explicit `mode=direct` remains original bytes. Cost is therefore one audio decode/encode on every playback and seek plus its lossy generation; force never spends a video encode solely on volume.

**Meter fanout is conditional.** Multichannel sources still need native and folded measurements. Mono and stereo do not: EBU's DualMono weighting is the duplicated mono fold, two-channel input is already the target layout, and max per-channel true peak is unchanged. Those sources run one meter and copy its answer into both stored facts. A forced two-meter regression proves both LUFS and dBTP agree before this shortcut is allowed; roughly 40% of the pending movie audio-hours avoid duplicate EBU work.
...
**Where it runs.** The hub remains the orchestrator and authority: it chooses seasons in watched-first order, resolves the same connected playback-ranked exact source for each episode, handles the chapter-only fast path, and stores `media_segments` / `media_segment_scans`. Chapter meaning is normalized when a file or later chapter declaration lands: `files.chapter_segment_kinds` records recap/intro/credits bits beside the classifier generation, and startup rebuilds null or stale masks from stored `streams_json` in pure Rust. The scheduler can therefore preserve genuinely chapter-complete seasons across an incompatible mediahost while skipping generic scene chapter lists with one integer test; rebuilding opens no source and point-of-need selection parses no chapter JSON. For inferred boundaries it sends the ordered exact-source season over the mediahost control link. The mediahost validates each size/mtime, runs the unchanged `kahawai-intro` comparison against local paths, re-stats the sources, and returns only boundaries. This removes the measured ~500 MB and ~one LAN minute per episode from the byte plane. A protocol-minor and detector-generation gate leaves work pending on old mediahosts rather than silently falling back to leases. Existing completed rows remain detector generation 2 because execution moved but the algorithm did not.
...
**Scanner.** Bounded-concurrency walker (`ignore` crate for traversal, N=4 discoverers) feeding a work queue persisted in a small local SQLite journal so scans resume (MH-7). The configured collection name is its stable per-mediahost ID. A root token is `root-sha256-` plus unpadded base64url of the complete `SHA-256(utf8("kahawai-root-path-v1") || 0x00 || normalized_path_utf8)`; paths are made absolute relative to the config file and normalized lexically, never filesystem-canonicalized. The source key is `(module, collection, root token, path_rel)`, so root list order cannot alias files. Each file: fast-path check → if changed, `GstDiscoverer::discover_uri` with 30 s timeout → map `GstDiscovererInfo` into `StreamInfo` (codec caps → normalized codec enum + profile/level from caps fields; HDR from mastering-display/CLL caps and DOVI configuration boxes; tags via `GstTagList` for music). Sidecar association by stem-matching within the directory (MH-4). Failures → `FileError` with the GStreamer diagnostic (MH-8).

**Watcher.** `notify` events debounced 2 s, coalesced per directory, feeding the same queue; nightly reconciliation walk catches missed events (network mounts). Cross-collection overlaps fan one event into every matching namespace. A watcher or scan failure for one unavailable root reports that exact root and leaves its previous manifest rows seen, so reconciliation neither deletes that mount's catalogue nor substitutes another root; other roots continue and later sweeps retry it.

**Collection-scoped exact sources and root adoption (protocol 3).** Protocol 3 deliberately rejects every protocol-2 mediahost and transcoder during `Hello`; operators upgrade all satellite binaries before reconnecting them. It has one required `SourcePath { root_token, path_rel }` shape for records, manifests, worklists, extraction results and reads, and root announcements carry the token beside the normalized configured path so the hub validates their binding. Direct migration 53 replaces unpublished migrations 53–56: each item gains explicit collection ownership, every old file/source pair becomes one stable `files.id`, source-bound subtitles and image-failure memory reference that ID, and libraries remain composition only. The migration neither guesses roots nor scans, calls providers, changes generations or regenerates caches; on the real level-52 catalogue it completes in about 1.5 s with all 52,982 tracks and 2,837 lineage links preserved. Migration 54 removes the remaining duplicate subtitle owner: embedded/sidecar tracks and physical derivatives carry only `source_id`, downloaded/manual tracks carry only `item_id`, and a derivative inherits its parent's owner. File rebinding therefore moves the whole physical track tree without trigger-maintained rewrites; source deletion evicts reproducible OCR/raster rows. Migration 57 records explicit playable renditions: every ordinary file is one source, while multipart files group only within the same collection, root, directory and release-family basename. Completeness is durable through `expected_parts`; candidates are ranked and admitted as whole renditions, never reconstructed by mixing equal ordinals from different editions. Migration 59 removes the transitional `files.item_id` and `files.part` columns in place, so `files` now stores physical facts only and all logical ownership/order comes from `playable_sources` and `playable_source_parts`; the in-place drop deliberately avoids firing dependent-row cascades that a table rebuild would trigger.

A validated announcement inserts `collection_roots`. One configured root proves every legacy root-less file in that collection, so one indexed `UPDATE files SET root_id=…` in `BEGIN IMMEDIATE` adopts all of them; dependent records already follow the stable file ID and need no rewrite. A minimal `root_adoption_pending` bit makes suppression crash-safe: protocol 3 repeats the harmless in-sync response until acknowledgement, then the mediahost immediately retries the consumed startup trigger. Matching generations skip; a real mismatch enters normal incremental reconciliation. For a pre-migration multi-root collection, the hub sends only unresolved `(path, size, head/tail fingerprints, oshash)` rows. The mediahost checks exact candidate paths under each root; exactly one complete match adopts that file ID, while zero or several matches remain blocked with a diagnostic. Adoption performs no directory walk, provider work, rematch, scan or generation advance. Exact source/cache keys prevent cross-root collisions, and expensive legacy cache artifacts are promoted only where identity is unambiguous.
...
**Job runners (MH-11/MH-13).** Urgent subtitle/image requests run immediately because a viewer is waiting. ED2K, declarations, geometry/keyframe probes and subtitle pre-warm remain idle-gated background work. Season analysis has its own one-at-a-time queue because its unit is a whole ordered season rather than one source; it waits while a scan or viewer lease is active and checks that gate between analyzer steps. Loudness has a separate one-file-at-a-time runner and shares the single background permit, but it is lower priority: queuing a watched-first season marks segment work pending, the loudness GStreamer callback releases the permit at its next audio-buffer checkpoint and blocks there, and the same pipeline reacquires the permit and resumes after the season. No audio is re-decoded, segment analysis does not overlap the two loudness meter branches, and the paused pipeline's bounded queues are the memory cost of preserving completed work. File/track start, completion time and segment yield/resume are logged so a long decode is visible rather than appearing idle. All runners keep control-link intake and heartbeats independent of blocking file work.
...
**File server.** Serves `OpenRead` leases only. Each protocol-3 request carries the persisted collection and required exact `SourcePath`; the mediahost resolves that one configured root and canonicalizes the candidate only for confinement, never searches other roots. Missing sources and empty/unknown root tokens are protocol errors. Opens remain read-only; canonical confinement rejects `..` and symlink escapes (NFR-4).
