...
Sidecar `.ass` files have no demuxer, so they need the same stream synthesised from the composed script the hub already stores — the inverse of what `subindex` does when it composes one. **Fonts are the part that nearly rules sidecars out.** `assrender` exposes no font directory; its only control is `embeddedfonts`, and it collects fonts from GST_TAG_ATTACHMENT samples arriving as a tag event on `text_sink`. An `appsrc`-fed script carries no such tags, and without the fonts libass substitutes and typeset signs render wrong — quietly defeating the reason someone chose burn over flatten. It works, but the sample has to be shaped exactly as matroskademux shapes it: buffer = font bytes, caps = the font mimetype, **and an info structure carrying `filename`**. Without the filename the tag event still arrives and `handle_tags` still runs, but no font is registered and the failure is silent. Verified by watching `gst_ass_render_handle_tag_sample` fire.
