...
Sidecar `.ass` files have no demuxer, so they need that stream synthesised from the composed script the hub already stores — the inverse of what `subindex` does when it composes one.

*Which makes the path depend on origin, and the font problem disappear.* Fonts exist in exactly two places: as attachments inside the mkv, or nowhere. A standalone `.ass` references fonts by name and relies on the system, which is what every other player does with it — so there is nothing to deliver and nothing lost. **Embedded tracks therefore take the demuxer's own subtitle pad**, which carries the attachments with it and needs no synthesis and no reconstruction of the script; **sidecars take `appsrc`** and go without fonts, as they would anywhere else. Feeding an embedded track through `appsrc` from our extracted copy would throw away attachments the demuxer was about to hand us — a self-inflicted problem, and the reason this is two paths rather than one.
...
*After a seek* the demuxer pad starts mid-stream, so a line already on screen at the seek point is missing until the next one. The same shape as the PGS gap HUB-32b answers with a pre-read timeline, but ASS lines last seconds rather than persisting, so the miss is brief and not worth pre-reading for.
