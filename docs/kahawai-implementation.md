...
Decision order per HUB-16: try full direct play; else keep every stream that fits and remux; else encode only failing streams. Rules include: profile/level comparison for H.264/HEVC/AV1; HDR→SDR tone-map (HUB-15a) when client lacks HDR — element candidates in preference order: `vapostproc` with HDR tone-mapping on VA-API boxes, a libplacebo-backed GStreamer path where its integration is available, and a shader/`glcolorconvert` fallback; the transcoder reports which (TC-1 filters), the doctor prints the per-box verdict, and negotiation only forces the encode when a capable box exists — otherwise the stream is delivered unmapped with the verdict saying so. Delivery is split across two arcs: the negotiation arc ships detection (MH-3 colorimetry probing), the profile's `hdr` capability and the decision arm inert (verdict-note only); the HUB-15a arc lands the element chain and the quality bar against a real HDR10/HLG test matrix, then flips the arm to force-encode. Further rules: channel downmix when layout unsupported; text subs converted SRT↔WebVTT with `subparse`; ASS handled per the `ass_fallback` policy (§4.3b: `ClientRender` when the client declares it, else `Burn` or `Flatten` — and after a `Flatten` substitution the plan is re-evaluated, since removing the burn-in often demotes a video encode back to remux/direct); PGS/VOBSUB → `BitmapStream` when the client declares `graphics_overlay` (server-side decode to a timed bitmap track, no video work, §4.3b), `Ocr` when the tier is available and preferred (client can't composite, or the user's per-session/remembered preference selects text over tiles — an automatic bandwidth threshold is deferred until the hub measures client bandwidth, alongside the quality-ladder machinery), else burn-in — piggybacked when the plan already encodes video, otherwise forcing a video encode only as the true last resort; bandwidth cap forces a ladder whose top rung ≤ cap.
