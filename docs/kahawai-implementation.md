...
Implementations: `thetvdb` (v4 API, JWT login flow), `tmdb`, `musicbrainz` (+ Cover Art Archive), `local` (NFO + sidecar art + embedded tags). Providers compose into per-media-type chains with **first-claim-wins** field merging (HUB-5) — the earliest provider to supply a field owns it. Normative defaults: anime → `[anime-native, tmdb, tvdb]` (episode/artwork bridged via mapped IDs, §4.3b — never independently re-matched), movies/series → `[tmdb, tvdb]`, music → `[embedded tags read at resolution time, musicbrainz]`. The `local` provider, when configured, is a chain-front insertion (HUB-9), repositionable per library. A background refresher re-fetches continuing series weekly.
...
**Disk bounds (OPS-6) — no cache eviction, deliberately.** Every persistent cache the hub keeps turns out to be expensive or impossible to rebuild, so evicting any of it trades a scarce resource for a cheap one. **Extracted cues and font bundles** look like the obvious candidates and are the worst of them: rebuilding one demuxes the entire source file over a byte-plane lease — gigabytes pulled across the network for a few hundred KB of text. That cost is the whole reason the HUB-34 ladder exists; a janitor deleting those entries is a machine for re-paying it. **Downloaded subtitles** are database-referenced, shared between every user of the item (HUB-23) and cost a rate-limited provider entitlement. **Artwork and the AniDB dumps/XML** are provider-derived, and a re-fetch is exactly the traffic §4.3's pacing exists to avoid. What is left is transient, and already bounded by lifecycle: session scratch is wiped at startup, torn down per session, and idle-reaped.

The number that makes this concrete: a 37k-file library produced 2.4 GB of subtitle cache against terabytes of media it describes. Disk is not the scarce resource here — provider entitlements and mediahost I/O are. Should a deployment ever genuinely need a cap (hub `data_dir` on a small SD card), the honest design is an admin-triggered purge that says what it will cost, not a silent hourly janitor. Hub stream proxying uses fixed bounded buffers (64 KiB chunks, bounded channel per session) so a slow client applies backpressure to the mediahost read instead of ballooning hub memory — this also caps per-session memory for the in-hub remuxer via `appsrc` `max-bytes`. Hub stream proxying uses fixed bounded buffers (64 KiB chunks, bounded channel per session) so a slow client applies backpressure to the mediahost read instead of ballooning hub memory — this also caps per-session memory for the in-hub remuxer via `appsrc` `max-bytes`.
