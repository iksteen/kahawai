...
**Where burn-in applies today.** The index walk is disk-speed on a local source and round-trip-bound over the byte plane — measured at ~4 KB/s hub→mediahost→NAS, so a walk that costs milliseconds locally does not finish inside a session's startup at all. The walk therefore runs under a read budget and, failing it, the session still starts and simply does not burn. Negotiation does not offer the tier it cannot deliver: `burn_capable` is a server-side fact (like the tone-map flag) set when the source is read locally (AR-11 short-circuit), and a remote source reports the track `unavailable` with the reason rather than promising a burn. Lifting that restriction means extracting image display sets on the MEDIAHOST, where the file is local and the same sparse walk is free, shipping the raw blocks (kilobytes) to the worker — the natural next step, and one that would also give the bitmap tier (HUB-32b streaming) a cache instead of requiring a live pipeline.
