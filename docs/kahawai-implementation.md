...
**How that is stored.** Each provider's answer is a row in `provider_metadata (item_id, provider, …)`; the row the API serves, `merged_metadata`, is the *merge* of those, recomputed by rank whenever an answer lands. Two conventions in those rows are easy to misread: `provider_id` is always that provider's real record id, including when the provider was only filling gaps — an *empty* `provider_id` means solely "looked, found nothing" and pairs with `confidence = 'miss'`, which is how never-ask-twice is recorded. The module doc on `hub/providers.rs` is the reference for what these tables mean. Identity is therefore decided by rank, not by which row has an id: the highest-ranked answer with a non-empty id owns `provider`/`provider_id`/`confidence` on the merged row. A consequence worth knowing before reordering: promoting a provider that had merely been describing an item also hands it that item's identity — intended, since "rank TMDB over AniDB" means exactly that. Keeping the losers is what makes precedence re-decidable: reordering providers re-merges from local rows and contacts nobody — which matters because the alternative is re-asking AniDB for every item, and that is how bans happen (§4.3). Order lives in `provider_ranks` (per media type, editable at runtime, `POST /admin/v1/providers/chains/{media_type}`) and only a permutation of the known set is accepted, so no provider can be silently dropped. Rank 0 is reserved for a human: a manual match keeps its provider attribution but is marked `confidence='manual'`, which outranks every chain position, so neither a reorder nor a later run can undo a correction (HUB-8, HUB-30a).

Because a match no longer ends the walk, the chain continues while fields are still empty and stops the moment the row is complete — the common case (TMDB answers everything) still costs exactly one provider's traffic. Two rules keep the tail honest: a provider that has already answered is never asked again, its miss included, so an item where nobody has a synopsis does not re-ask forever; and a provider that finds someone else owns the identity may only *describe*, never re-identify — for anime that means bridging through the mapped TMDB/TVDB id and declining outright when no mapping exists. What cannot be asked right now — a ban, a 429, a dead link — lands in `enrichment_queue` with a backoff (15 min → 1 h → 4 h → 24 h) and comes back due, so provider unavailability costs a delay rather than a permanently blank item. Normative defaults: anime → `[anime-native, tmdb, tvdb]` (episode/artwork bridged via mapped IDs, §4.3b — never independently re-matched), movies/series → `[tmdb, tvdb]`, music → `[embedded tags read at resolution time, musicbrainz]`. The `local` provider, when configured, is a chain-front insertion (HUB-9), repositionable per library. A background refresher re-fetches continuing series weekly.
...
**Image subtitles — OCR text tier (HUB-32c).** Between bitmap and burn-in sits an OCR tier: the hub converts the image stream to a plain text track, for clients that can't composite (better than forcing a burn-in encode) and for constrained links (a text track is a few KB; even the bitmap tile track is orders of magnitude heavier — this is the tier you want on a high-latency remote session). In v1 the text-over-tiles choice is a user preference — selectable per session, remembered per user; an automatic bandwidth threshold waits for hub-side bandwidth measurement, which lands with the quality-ladder machinery. Pipeline: HUB-34 ladder materializes the stream → VobSub goes through `subtile-ocr`'s native path; PGS reuses our display-set decoder from the bitmap tier, feeding the same preprocessing (black-on-white normalization, per-line split for PSM 7) into Tesseract via `leptess` → SRT, normalized into the subtitle store with `origin: ocr`, language model chosen from the track's language tag, cached per `(stream, lang, engine_version)`. Latency handling: a cached result serves instantly; on a cold miss the hub generates with a ~10 s budget (typical tracks OCR in seconds) and, if exceeded, the session proceeds on the next tier per policy while OCR finishes in the background for next time. OCR output is marked as machine-derived in the API and player UI (imperfection is expected — Tesseract will misread stylized fonts), and users can review, regenerate, or delete it like any subtitle.

Feature gating: cargo feature `ocr` on `kahawai-hub`, **default-on**, gating the `subtile-ocr`/`leptess` dependency — `--no-default-features` builds have no Tesseract linkage for minimal deployments. Runtime, the hub probes Tesseract and enumerates installed trained models at startup; negotiation only offers the `Ocr` outcome for languages with a model present, and `doctor` reports the exact gap ("no `jpn` traineddata → OCR unavailable for Japanese"). Licensing (resolved, NFR-8): Kahawai is MIT; `subtile-ocr` is GPL-3.0, so release binaries with `ocr` enabled are combined works under GPL-3.0 terms — release artifacts note this, and the `--no-default-features` build stays copyleft-free for embedders.
...
What is left is transient and already bounded by lifecycle: session scratch is wiped at startup, torn down per session, and idle-reaped. So there is no janitor. Disk is not the scarce resource here — provider entitlements, mediahost I/O and interaction latency are. Should a deployment genuinely need a cap (hub `data_dir` on a small SD card), the honest design is an admin-triggered purge that states what it will cost, not a silent hourly sweep. Hub stream proxying uses fixed bounded buffers (64 KiB chunks, bounded channel per session) so a slow client applies backpressure to the mediahost read instead of ballooning hub memory — this also caps per-session memory for the in-hub remuxer via `appsrc` `max-bytes`. Hub stream proxying uses fixed bounded buffers (64 KiB chunks, bounded channel per session) so a slow client applies backpressure to the mediahost read instead of ballooning hub memory — this also caps per-session memory for the in-hub remuxer via `appsrc` `max-bytes`.
...
**Planned (post-v1, spec'd).** OCR subtitle tier (HUB-32c, §4.3b): `ocr` cargo feature, `subtile-ocr`/Tesseract integration, `Ocr` negotiation outcome, cache + regeneration UI, doctor checks. License resolved (NFR-8): project is MIT; `ocr`-enabled release binaries ship under GPL-3.0 terms as combined works, stated on the release artifacts.
